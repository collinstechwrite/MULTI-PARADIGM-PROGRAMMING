...
def print_customer(c):
    ...
    if len(out_of_stock) == 0:
        if float(c.budget) >= float(total):
            ...
            print(f"{c.name}'s budget is {c.budget}") #Correct
            print(f"{c.name} has enough money") #Correct
            print(f"total of {total} will be deducted from {c.name}'s budget") #Correct

            # Missing John's budget is ... * the altered budget


            print(f"total of {total} added to shop") #Correct
            print(f"Shop has {s.cash}") #Correct

            # At this point the program should be updating the customer budget variable and shop  cash variable with the processed order

           # now we loop back and print shop stock and the menu
            ...
            s = Shop()
            print('--------------------------------------------------')
            print('FAIL CUSTOMER DOES NOT HAVE ENOUGH MONEY !') # Customer does not have enough money ? I have edited this description
            print(f"{c.name}'s budget is {c.budget}") # Correct
            #Missing The total cost of all items is ...! * the total cost of all items

            print(f"{c.name} doe not have enough money!") # c.name does not have enough money! I have edited the description
            print(f"total of {total} will be deducted from {c.name}'s budget") # total of 0 will be deducted from c.name budget 

            print(f"total of {total} added to shop") #total of 0 added to shop
            print(f"Shop has {s.cash}") # should be the shop's current cash

            # No need to update customer budget or shop cash because the order isn't processed
            # now we loop back and print shop stock and the menu
...
1) for live mode
1 ) ../customer_too_much_bread.csv
2 ) ../customer.csv
3) ../customer_not_enough_money.csv
0) Exit
